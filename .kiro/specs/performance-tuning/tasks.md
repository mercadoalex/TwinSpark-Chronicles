...
- [ ] 3. FaceCropCache — in-memory LRU cache
  - [-] 3.1 Implement `FaceCropCache` in `backend/app/services/face_crop_cache.py`
...
- [ ] 4. CacheManager — coordinator, cleanup loop, and stats endpoint
  - [ ] 4.1 Implement `CacheManager` in `backend/app/services/cache_manager.py`
  ...
  - [ ] 4.2 Add `GET /api/cache/stats` endpoint to `backend/app/main.py`
...
- [ ] 6. Integrate caching into PhotoService and FaceExtractor
  ...
  - [ ] 6.2 Integrate `FaceCropCache` into `FaceExtractor.extract_faces`
  ...
  - [ ] 6.3 Add cache invalidation to `PhotoService.delete_photo`
